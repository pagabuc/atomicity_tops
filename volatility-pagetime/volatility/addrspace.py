...
class BaseAddressSpace(object):
    ...
    def __init__(self, base, config, *_args, **_kwargs):
        ...
        # We save the Pagetime object in the config class, which is
        # singleton.  Otherwise every time an AS is
        # instantiated, all the pagetime informations are reloaded.
        # XXX: Probably there's a more elegant way to do it
        if not config.PAGETIME:
            self.pt = None
            return

        if not hasattr(config, "pt"):
            import volatility.plugins.linux.pagetime as pagetime
            config.pt = None
            pagetime_file = config.LOCATION.replace('file://', '') + '.times'
            try:
                config.pt = pagetime.Pagetime(pagetime_file)
            except IOError as e:
                print('[WARNING] Pagetime file %s not found, but --pagetime specified' % pagetime_file)
                sys.exit(-1)
                
        self.pt = config.pt

    def pagetime(self, paddr):
        if not self.pt:
            return 0
        return self.pt.pagetime(paddr)

    def vpagetime(self, vaddr):
        paddr = self.vtop(vaddr)
        return self.pagetime(paddr)

    def track_pagetime(self, paddr):
        if self.pt:
            t = self.pagetime(paddr)
            self.pt.track(t)

    def vtrack_pagetime(self, vaddr):
        if self.pt:
            paddr = self.vtop(vaddr)
            self.track_pagetime(paddr)

    def print_pagetime(self):
        self.pt.display()
