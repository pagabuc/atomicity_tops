...
class LimeAddressSpace(addrspace.AbstractRunBasedMemory):
    ...
    def read(self, addr, length):
        # This handles direct memory reads i.e. pagetables, read(task.mm.arg_start)...).
        if addr != 0:
            self.track_pagetime(addr)
            
        return addrspace.AbstractRunBasedMemory.read(self, addr, length)
